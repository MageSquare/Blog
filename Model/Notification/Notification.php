...
use MageSquare\Blog\Model\Comments as CommentsModel;
...
class Notification
{
    const NOTIFICATION_TYPE = 'msblog/comments/notify_about_replies';
    ...
    public function notifyCustomersReplies(CommentsModel $comment): void
    {
        ...
        if ($template && $receivers && $sender) {
            $vars = [
                'post_link'    => sprintf('%s#ms-blog-comment-%d', $post->getUrl(), $replyToId),
