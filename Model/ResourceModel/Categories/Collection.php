...
namespace MageSquare\Blog\Model\ResourceModel\Categories;
...
class Collection extends \MageSquare\Blog\Model\ResourceModel\Abstracts\Collection
{
    ...
    const CATEGORY_POST_RELATION_TABLE = 'magesquare_blog_posts_category';
    ...
    {
        $this->_init(
            \MageSquare\Blog\Model\Categories::class,
            Categories::class
        );
    }
    ...
    public function getPostsCount($stores)
    {
        $select = $this->getConnection()->select()
            ->from(
                ['posts_cat' => $this->getTable('magesquare_blog_posts_category')],
                ['category' => 'posts_cat.category_id', 'posts_count' => 'COUNT(posts_cat.category_id)']
            )->join(
                ['posts' => $this->getTable('magesquare_blog_posts')],
                'posts.post_id = posts_cat.post_id AND posts.status = 2',
                []
            )->join(
                ['posts_store' => $this->getTable('magesquare_blog_posts_store')],
